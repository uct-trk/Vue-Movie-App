      ...
      <div class="ml-20">
        ...
        <div class="flex">
          ...
          <span class="ml-2 text-gray-400 text-sm"
            >{{ this.actor.birthday }} {{ this.actor.deathday }}</span
          >
        </div>
...
export default {
  ...
  methods: {
    async fetchActor(actorId) {
      await axios
        .get(
          `https://api.themoviedb.org/3/person/${actorId}?api_key=d9a58f68b032567df17dbf974fe13148`
        )
    ...
    async fetchCredits(actorId) {
      await axios
        .get(
          `https://api.themoviedb.org/3/person/${actorId}/combined_credits?api_key=d9a58f68b032567df17dbf974fe13148`
        )
    ...
    async fetchSocial(actorId) {
      await axios
        .get(
          `https://api.themoviedb.org/3/person/${actorId}/external_ids?api_key=d9a58f68b032567df17dbf974fe13148`
        )
        .then((res) => (this.socialDetails = res.data))
        .catch((err) => console.log(err));
    },
    castDetails(cast) {
